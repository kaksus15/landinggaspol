...
@section('title', 'Desa Binaan')

@section('contents')
    <div class="card card-primary card-outline">
        <div class="card-header">
            <h3 class="card-title">
                <button type="button" class="btn btn-default btn-sm" onclick="addForm('{{ route('binaan.store') }}')">
                ...
                <form method="POST" id="simpan" onsubmit="event.preventDefault();">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="" class="form-control-label">Instansi</label>
                            <div style="width: 100%;" class="mr-2">
                                <select name="instansi_id" id="instansi_id" onchange="pilihDesaBinaan()"
                                    class="flex-grow-1 mr-2 form-control custom-select select2">
                                    @foreach ($instansi as $inst)
                                        <option value="{{ $inst->id }}">{{ $inst->instansi }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div style="width: 100%;" class="mr-2">
                                <select name="desa_id[]" id="desa_id"
                                    class="flex-grow-1 mr-2 form-control custom-select select2" multiple="multiple">
                                    @foreach ($desa as $ds)
...
@push('scripts')
    ...
    <script src="{{ asset('/template/dist/js/pages/desabinaan.js') }}"></script>
@endpush
